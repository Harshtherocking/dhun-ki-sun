...
#         for thaat_idx, thaat_name in enumerate(sorted(os.listdir(root_dir))):
#             thaat_path = os.path.join(root_dir, thaat_name)
#             if not os.path.isdir(thaat_path):
#                 continue

#             # Map Thaat to an integer
#             self.thaat_to_idx[thaat_name] = thaat_idx
...
#                 self.raga_to_idx[(thaat_name, raga_name)] = raga_idx
...
#                                 self.data.append((audio_path, thaat_idx, raga_idx))

#     def __len__(self):
#         return len(self.data)

#     def __getitem__(self, idx):
#         audio_path, thaat_label, raga_label = self.data[idx]
...
#     # Create DataLoader
#     trf_loader = DataLoader(trf_dataset, batch_size=1, shuffle=True)

#     # Inspect a batch and plot a waveform
#     for x, thaat_y, raga_y in trf_loader:
#         print("Waveform (x):", x.shape)        # Should be (batch_size, channels, samples)
#         print("Thaat Labels (y1):", thaat_y)   # Integer labels for Thaat (Parent Category)
#         print("Raga Labels (y2):", raga_y)     # Integer labels for Raga (Child Category)
